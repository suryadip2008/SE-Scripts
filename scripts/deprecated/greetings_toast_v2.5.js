...
function displayToast() {
    ...
    if (day === bDay && month === bMonth) {
        ...
        if (hours < 12) {
            predefinedPrompt = getRandomElement(greetingPresets.morning).replace('{username}', username);
        } else if (hours < 18) {
            predefinedPrompt = getRandomElement(greetingPresets.afternoon).replace('{username}', username);
        } else {
            predefinedPrompt = getRandomElement(greetingPresets.evening).replace('{username}', username);
        }
    }
